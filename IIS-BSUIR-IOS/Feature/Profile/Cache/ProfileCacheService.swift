...
final class ProfileCacheService {
    ...
    func save<T: Encodable>(_ value: T, forKey key: String) {
        ...
        if let existing = fetchEntry(forKey: key) {
            ...
            modelContext.insert(CachedProfileEntry(key: key, data: data))
        }
        try? modelContext.save()
    }

    private func fetchEntry(forKey key: String) -> CachedProfileEntry? {
        let predicate = #Predicate<CachedProfileEntry> { $0.key == key }
